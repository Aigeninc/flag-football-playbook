...
(function () {
  ...
  // Coach Call Sheet state
  let coachMode = false;
  let situation = { down: null, field: null, defense: null };
  ...
  function buildControls() {
    ...
    coachBtn.addEventListener('click', () => {
      coachMode = !coachMode;
      coachBtn.style.opacity = coachMode ? '1' : '0.4';
      document.getElementById('coach-panel').style.display = coachMode ? 'block' : 'none';
      if (coachMode) setupCoachPanel();
    });
  ...
  // ── Coach Call Sheet Engine ──────────────────────────────
  // Situational play scoring: each play gets a score 0-100 based on situation
  ...
  function getPlayScore(playName) {
    const sit = PLAY_SITUATIONS[playName];
    if (!sit) return 0;
    let score = 50; // base
    let factors = 0;
    if (situation.defense && sit.vs[situation.defense]) {
      score += sit.vs[situation.defense] - 50;
      factors++;
    }
    if (situation.down && sit.down[situation.down]) {
      score += (sit.down[situation.down] - 50) * 0.7;
      factors++;
    }
    if (situation.field && sit.field[situation.field]) {
      score += (sit.field[situation.field] - 50) * 0.8;
      factors++;
    }
  ...
  function setupCoachPanel() {
    // Bind situation buttons
    document.querySelectorAll('.sit-btn').forEach(btn => {
      // Set active state from current situation
      const cat = btn.dataset.sit;
      const val = btn.dataset.val;
      if (situation[cat] === val) btn.classList.add('active');

      btn.addEventListener('click', () => {
        const category = btn.dataset.sit;
        const value = btn.dataset.val;
        // Toggle: if already selected, deselect
        if (situation[category] === value) {
          situation[category] = null;
        } else {
          situation[category] = value;
        }
        // Update button states
        document.querySelectorAll(`.sit-btn[data-sit="${category}"]`).forEach(b => b.classList.remove('active'));
        if (situation[category]) btn.classList.add('active');
        updateCoachRecs();
      });
    });
    updateCoachRecs();
  ...
  function updateCoachRecs() {
    const recsDiv = document.getElementById('coach-recs');
    recsDiv.innerHTML = '';

    // No situation selected yet
    if (!situation.down && !situation.field && !situation.defense) {
    ...
    scored.slice(0, 5).forEach((rec, rank) => {
      ...
      div.addEventListener('click', () => {
        selectPlay(rec.idx);
        // Also add to queue if queue is active
        if (queueMode) {
          queue.push({ playIdx: rec.idx, result: null });
          queuePos = queue.length - 1;
          renderQueue();
        }
      });
